...
ironton <- read.csv("no_dis03336890.csv")
...
temp_plot = ggplot(ironton, aes(x = ironton$X_00060_00003 , y = ironton$X_99133_00003, col=Month)) +
  ...
  labs(x="Discharge (mm/hr)", y = "Discharge (mg/L)") +
  theme_classic() +
  theme(text = element_text(size=20)) +
  theme(aspect.ratio = 1) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)), limits=c(10^-2, 10^2.0)) +
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)), limits=c(10^-3.0, 10^4.0)) +
  annotation_logticks()
temp_plot
ggsave(temp_plot, file=paste0("plot_norm_", "03336890_all_plot.pdf"), width = 14, height = 10, units = "cm", useDingbats=FALSE)
...
Sept = ggplot(ironton[ironton$Month == "September", ], aes(x = Qnorm9, y = value9)) +
  ...
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)), limits=c(10^-3, 10^4.0)) +
  annotation_logticks()
...
Oct = ggplot(ironton[ironton$Month == "October", ], aes(x = Qnorm10, y = value10)) +
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)), limits=c(10^-2, 10^2)) +
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)), limits=c(10^-3, 10^4)) +
  annotation_logticks()
...
apr_aug
sept_dec
ggsave(jan_mar, file=paste0("plot_norm_", "03336890_jan.pdf"), width = 14, height = 10, units = "cm", useDingbats=FALSE)
ggsave(apr_aug, file=paste0("plot_norm_", "03336890_apr.pdf"), width = 14, height = 10, units = "cm", useDingbats=FALSE)
ggsave(sept_dec, file=paste0("plot_norm_","03336890_sept.pdf"), width = 14, height = 10, units = "cm", useDingbats=FALSE)
